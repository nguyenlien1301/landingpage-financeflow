...
let handleMenuMobile = ()=> {
    ...
    const navMenu = document.querySelector(".header .header__left .header__left-nav");
...
let handlePopup = ()=> {
    ...
    const iframe = document.querySelector(".popup .popup__container .popup__container-video iframe");
    const popup = document.querySelector(".popup");
    const close = document.querySelector(".popup .popup__container .popup__container-video .close");
    const popupContainer = document.querySelector(".popup__container");
    if(popup) {
        let handleDataVideo = ()=> {
            let getDataVideo = videoThumb.getAttribute('data-video');
            iframe.setAttribute('src', `https://www.youtube.com/embed/${getDataVideo}?autoplay=1`)
        }
        let removePopup = ()=> {
...
let handleValidateForm = ()=> {
    ...
    let handleFormSub = ()=> {
        ...
        const input = document.querySelector(".subscribe__form .subscribe__form-group .input");
    ...
    postsTab.forEach((item, index) => {
       item.addEventListener('click', ()=> {
            ...
            if(!getNewList.classList.contains('active')) {
                document.querySelector(".posts .wrapper .news__list.active").classList.remove('active');
                getNewList.classList.add('active');
                document.querySelector(".posts__tab-item.is-tab-active").classList.remove('is-tab-active');
...
let handlePagination = ()=> {
    const pagingItem = document.querySelectorAll(".posts__pagination-paging li a");
    ...
    if(postsPagination) {
        pagingItem.forEach((item, index)=> {
            item.addEventListener('click', (e)=> {
                ...
                if(!item.classList.contains('is-active')) {
                    document.querySelector(".posts__pagination-paging li a.is-active").classList.remove('is-active');
        ...
        function updateBtn() {
            ...
            btnNext.classList.toggle('is-disable', currentIndex === pagingItem.length - 1);
        }
    }
}
